...
class RSSI_Scan(object):
    ...
    def getFrequency(raw_cell):
        signal = raw_cell.split('Frequency:')[1]
        signal = float(signal.split(' ')[0])
        return signal
    
    @staticmethod
    def getadress(raw_cell):
        signal = raw_cell.split('Address: ')[1]
        signal = str(signal.split(' ')[0]).strip('\n')
        return signal
    ...
    def parseCell(self, raw_cell):
        cell = {
            ...
            'address' : self.getadress(raw_cell)
